...
let every: number[] = [2, 4, 6, 8, 10];

// Using every to check if all elements are even
let allEven: boolean = every.every(function (element) {
    return element % 2 === 0;
});
...
let some: number[] = [1, 3, 5, 7, 8];

// Using some to check if at least one element is even
let hasEven: boolean = some.some(function (element) {
    return element % 2 === 0;
});
...
let sorts: string[] = ['banana', 'apple', 'orange', 'grape'];
sorts.sort();
...
//  modifies the original array in place, meaning the original array's order is changed.
    ...
    let reverse: string[] = ['apple', 'banana', 'orange', 'grape'];
    let reversed: string[] = reverse.reverse();
